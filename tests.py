...
class BaseTestCase(AsyncHTTPTestCase):
    ...
    def setUp(self):
        ...
        efq.CHARACTERS = {}
        with ExpectLog('efq', 'Creating character test 1'):
            self.testchar1 = get_test_char('test 1', '1000000001', VINDI_FIT)
        with ExpectLog('efq', 'Creating character test 2'):
    ...
    def httpclient_patch(self, body, code=200):
        future = Future()
        future.set_result(Mock(code=code, body=body))
        return patch('efq.http', fetch=Mock(return_value=future))

    def redis_patch(self, value=None, method='get'):
        kwargs = {}
        if value is not None:
            ...
            kwargs['side_effect'] = getattr(efq.redis, method)
        return patch('toredis.Redis.%s' % method, **kwargs)
    ...
    def fetch(self, *args, **kwargs):
        ...
        response = super(BaseTestCase, self).fetch(*args, **kwargs)
        if response.code == 599:
            raise response.error
        return response

    def get_headers(self, headers=None, char=None, eve_trust=True, logged_in=None, uuid=None):
        if headers is None:
            headers = HTTPHeaders()
        if eve_trust:
            if char is not None:
        ...
        if logged_in:
            if 'Cookie' in headers:
                ...
                cookies = []
            if uuid is None:
                uuid = efq.BaseHandler.uuid
            key = TEST_APP_SETTINGS['cookie_secret']
            headers.add('Cookie', '; '.join(cookies + [
                '='.join(i) for i in [
                    ('character', native_str(create_signed_value(key, 'character', char.name))),
                    ('uuid', native_str(create_signed_value(key, 'uuid', uuid))),
                ]
            ]))
        return headers
...
def faketoken():
    orig_generate_token = efq.generate_token
    def side_effect(x):
        side_effect.token = orig_generate_token(x)
        return side_effect.token
    side_effect.token = None
    mock = patch('efq.generate_token', side_effect=side_effect)
    return mock
...
class TestAccess(BaseTestCase):
    ...
    def test_channel_auth(self):
        ...
        def login(char, cookies=None, bad_token=False):
            ...
            if bad_token:
                sent_token = efq.generate_token(int(len(token) / 2))
                assert token != sent_token
            else:
                sent_token = token

            s = EVELIVE_LINE % (char.name.replace(' ', '_'), char.name, sent_token)
            with self.httpclient_patch(s) as mock:
                ...
                if not bad_token:
                    ...
                    self.assertIsRedirect(response, '/login?channel_auth=failed')
                    response = self.fetch('/login?channel_auth=failed',
                                          headers=get_cookie_headers(cookies, response),
                                          char=char)
                    self.assertRespContains(response, token)
        ...
        cookies = login(self.testchar1, bad_token=True)

    def test_mail_auth(self):

        def actual_test(rp_get, rp_set, rp_exp, tp):
            ...
            response = self.fetch("/login/mail_auth", method='POST', body='')
            ...
            expected_event = {
                ...
                'token': tp.side_effect.token,
                'character': self.testchar1.to_json(),
            }
            poll_cb.assert_called_once_with(expected_event)
            k = 'efq:mail_auth_token:%s' % native_str(tp.side_effect.token)
            rp_set.assert_called_once_with(k, self.testchar1.name)
            rp_exp.assert_called_once_with(k, 600)
            self.assertIsNotNone(tp.side_effect.token)
            response = self.fetch("/login/token/%s" % tp.side_effect.token)
            rp_get.assert_called_once_with(k)
            self.assertIsRedirect(response, '/login/success')

        with patch('efq.BaseHandler.FCS', [self.testchar2]):
            with patch('efq.BaseHandler.ONLINE', set([self.testchar2])):
                with self.redis_patch(method='get') as rp_get:
                    with self.redis_patch(method='set') as rp_set:
                        with self.redis_patch(method='expire') as rp_exp:
                            with faketoken() as tp:
                                actual_test(rp_get, rp_set, rp_exp, tp)


class TestLogout(BaseTestCase):
    def test_logout(self):
        self.assertRespContains(self.fetch('/logout'), 'method="POST"')
        self.assertIsRedirect(self.fetch('/logout', method='POST', body=''), '/')
...
class TestCharacter(BaseTestCase):
    ...
    def test_get_character(self):
        ...
        with self.httpclient_patch(b'characterID="1111"') as hcp:
            with self.redis_patch('123:123;2::') as rp:
                ...
                hcp.fetch.assert_called_once_with('https://api.eveonline.com/eve/CharacterID.xml.aspx?names=qwe')
                rp.assert_called_once_with('efq:fitting:qwe')

        with self.httpclient_patch(b'characterID="1112"'):
    ...
    def test_invalid_character(self):
        with ExpectLog('efq', r"Can't get charid for zxc!"):
            with self.assertRaises(efq.CharacterNotFound):
                with self.httpclient_patch(b'characterID="0"'):
        ...
        with ExpectLog('efq', r"Request https://api.eveonline.com/eve/CharacterID.xml.aspx\?names=zxc failed:"):
            with ExpectLog('efq', r"Can't get charid for zxc!"):
                with self.assertRaises(Exception):
                    with self.httpclient_patch(b'blablabla'):
                        with self.redis_patch(''):
                            _ = yield efq.get_character('zxc')
    ...
    def test_redis_exception(self):
        with ExpectLog('efq', "Failed to get fit from redis!"):
            with self.httpclient_patch(b'characterID="1111"'):
                with self.redis_patch('') as rp:
...
class TestPoll(BaseTestCase):
    ...
    def test_uuid_cookie(self):
        with ExpectLog('efq', 'Character test 1 has bad uuid cookie'):
            self.assertRespContains(
                self.fetch('/poll', uuid='wrong_uuid'),
                '{"action": "reload"}',
            )
